...
export default function SellingListItem({ item, originalPrice, onPriceChange, onRemove }: SellingListItemProps) {
  ...
  return (
    ...
    <div className="mb-3 rounded-lg border bg-white p-4 shadow-sm xl:mb-0 xl:grid xl:grid-cols-12 xl:items-center xl:gap-2 xl:rounded-none xl:border-0 xl:border-b xl:border-gray-100 xl:bg-transparent xl:p-0 xl:py-2 xl:shadow-none dark:border-gray-700 dark:bg-gray-800 xl:dark:border-gray-700/50 xl:dark:bg-transparent">
      ...
      <div className="xl:col-span-6">
        <p className="text-lg font-bold text-gray-800 dark:text-gray-200">{item.name}</p>
        {item.imei && (
          <p className="mt-0.5 text-base font-bold text-gray-500 dark:text-gray-400">IMEI/SN: {item.imei}</p>
        )}
      </div>
      ...
      <div className="mt-4 flex items-end justify-between xl:mt-0 xl:contents">
        ...
        <div className="xl:col-span-2 xl:text-right">
          <label className="block text-xs text-gray-500 xl:hidden">ราคาปลีก</label>
          <span
            className={`text-base font-semibold ${
              isDiscounted ? "text-gray-400 line-through dark:text-gray-500" : "text-gray-700 dark:text-gray-300"
            }`}
          >
        ...
        <div className="xl:col-span-3 xl:px-4">
          <label className="block text-xs text-gray-500 xl:hidden">ราคาขาย</label>
          <input
            inputMode="numeric"
            value={item.unitPrice}
            onChange={handlePriceInputChange}
            className={`w-28 rounded-md border text-center text-base font-semibold focus:border-blue-500 focus:ring-1 focus:ring-blue-500 xl:w-full ${
              isDiscounted
