...
export default function DiscountModal({ isOpen, onClose, initialDiscounts, onApplyDiscounts }: DiscountModalProps) {
  const [activeTab, setActiveTab] = useState<"promos" | "custom" | "scan">("promos");
  ...
  useEffect(() => {
    if (isOpen) {
      const promoIds = new Set(initialDiscounts.filter((d) => d.id.startsWith("promo-")).map((d) => d.id));
  ...
  const handleScanCode = () => {
    ...
    const foundPromo = MOCK_PROMOTIONS.find((p) => p.code?.toUpperCase() === codeToFind);
  ...
  const handleApply = () => {
    ...
    const selectedPromos = MOCK_PROMOTIONS.filter((p) => selectedPromoIds.has(p.id));
  ...
  return (
    ...
    >
      <div className="flex h-full flex-1 flex-col">
        {/* Header and Tabs */}
        <div className="shrink-0 border-b p-6 dark:border-gray-700">
          <h3 className="text-2xl font-bold text-gray-800 dark:text-white">จัดการส่วนลด</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6">
              ...
              {MOCK_PROMOTIONS.map((promo) => (
                ...
                >
                  <div className="flex-1">
                    <h4 className="font-semibold text-gray-800 dark:text-gray-100">{promo.name}</h4>
                    <p
                      className={`font-bold ${promo.type === "percentage" ? "text-green-600 dark:text-green-400" : "text-orange-600 dark:text-orange-400"}`}
                    >
                      {promo.type === "percentage" ? `ลด ${promo.value}%` : `ลด ${promo.value} บาท`}
                    </p>
                  </div>
                  <div
                  ...
                  >
                    {selectedPromoIds.has(promo.id) && <span className="text-white">✓</span>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {activeTab === "scan" && (
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <h4 className="text-xl font-semibold text-gray-700 dark:text-gray-200">สแกนหรือกรอกโค้ดส่วนลด</h4>
              <p className="text-gray-500 dark:text-gray-400">
                วางเคอร์เซอร์ในช่องด้านล่างแล้วใช้เครื่องสแกน หรือพิมพ์โค้ดแล้วกด Enter
              </p>
              <input
                ref={scanInputRef}
                type="text"
              ...
              />
              <Button onClick={handleScanCode} className="w-full max-w-sm py-3 text-lg">
                ยืนยันโค้ด
              </Button>
          ...
          {activeTab === "custom" && (
            <div className="flex flex-col gap-4">
              <p className="text-gray-600 dark:text-gray-400">กรอกจำนวนส่วนลดที่ต้องการใช้กับยอดรวมของสินค้านี้</p>
              ...
              <div>
                <label htmlFor="custom-amount" className="block text-sm font-medium text-gray-600 dark:text-gray-400">
                  จำนวนส่วนลด
                </label>
                <input
                  id="custom-amount"
        ...
        {/* Action Buttons */}
        <div className="grid grid-cols-2 gap-4 p-4 dark:border-gray-700 dark:bg-gray-900/50">
